        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const [key, value] of Object.entries(_schema)) {
                ...
                if (object[key] == undefined) {
                    if (typeof value == "object") if (value.default) object[key] = value.default;
                }
                ...
                if (valueOptions && value.required && formatedValue == undefined)
                    return reject(new Error(`SchemaError: Field '${fieldName}' is empty but required`));
            ...
            //Return Object
            if (Object.keys(filteredObject).length > 0) return resolve(filteredObject);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            array.forEach(async (item) => {
                if (typeof arrayEntity == "function") formatedArray.push(formatData(item, arrayEntity));
                    ...
                    try {
                        let formatedObject = await this.filterObject(item, arrayEntity),
                            reduce;
